...
from src.drafting_engine import DraftingEngine
...
class SECSpecialistAgent:
    ...
    def __init__(self):
        ...
        self.drafting_engine = DraftingEngine(self)
    ...
    def run_drafting(self, ticker, section="mda", form_type="10-K"):
        ...
        if section == "mda":
            # Use the new high-fidelity drafting engine
            historical_context = filing_data.get("sections", {}).get("mda", "No historical data.")
            draft = self.drafting_engine.draft_mda_section(
                filing_type=form_type,
                company=filing_data.get("company_name", ticker),
                transcript_data=transcript_data.get("full_content", "No transcript context."),
                previous_filing_text=historical_context
            )
        else:
...
*⚠️ Professional Disclaimer: This report is generated by an AI agent for informational purposes only. It does not constitute legal, tax, or financial advice. Consult qualified professionals for official guidance.*"""
    ...
    def generate(self, system_prompt, user_prompt):
        """Bridge for DraftingEngine to call LLM."""
...
def run_sec_audit(ticker, form_type="10-K"):
    ...
    return agent.run_audit(ticker, form_type=form_type)
